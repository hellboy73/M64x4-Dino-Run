...
;   ***  v1.1 26.12.2025 full version with VS detection, joystick support and sound          *** 
;   ********************************************************************************************
...
init:
		...
		LDI 0x9f JAS wrSN76489              		; mute audio channel 1
		LDI 0xbf JAS wrSN76489					    ; mute audio channel 2
		LDI 0xdf JAS wrSN76489						; mute audio channel 3
		LDI 0xff JAS wrSN76489						; mute audio channel 4
		JPS init_speedcopy
													; draw initial screen
...
game_over:
		...
		JPS play_dead_tune
		MIZ 60 Z1
	go_loop:															; wait 60 frames = 1 sec while continue playing tunes  
...
Score:					;* calculating and displaying game scores, always increase by 1 point only 
			...
			CLB _score		INB _game_level	JPS play_level_up_tune			; if yes, increase the game level
...
Control_section:		;* set dino state based on joystick 
	...
	cs_up:		LDB _up			CPI 1	BNE cs_down		MIB 3 _dino_state	JPS play_jump_tune 		JPA cs_exit		; set dino into jump stage
...
_cloud_speed:		 		4							; initial speed /frames per pixel
				...
				;******************	
Animate_ground:	;* calculate next ground position based on game speed
...
_pressed:  	0
					...
					;******************
					;* Audio handling procedures
					;******************
play_jump_tune: 								; set initial parameters to start playing "jump" tune
			MIB 1 _ch1_playing 					; set ch1 as playing
			MIB <audio_jump _ch1_pointer		; set pointer to beginning of "jump" tune
			MIB >audio_jump _ch1_pointer+1
			CLB	_ch1_wait_counter				; reset wait counter
			RTS

play_level_up_tune: 							; set initial parameters to start playing "level up" tune
			MIB 1 _ch2_playing 					; set ch2 as playing
			MIB <audio_level_up _ch2_pointer	; set pointer to beginning of "level up" tune
			MIB >audio_level_up _ch2_pointer+1
			CLB	_ch2_wait_counter				; reset wait counter
			RTS

play_dead_tune: 								; set initial parameters to start playing "dead" tune
			MIB 1 _ch3_playing 					; set ch3 as playing
			MIB <audio_dead _ch3_pointer		; set pointer to beginning of "dead" tune
			MIB >audio_dead _ch3_pointer+1
			CLB	_ch3_wait_counter				; reset wait counter
			RTS

					; ******************
audio_channel1:		; * plays queue for channel 1, to be called every frame
					; * data taken from audio_jump table
					; ******************
				LDB	_ch1_playing 		CPI 0 BEQ ch1_exit		; if its not playing then exit
				LDB _ch1_wait_counter	CPI 0 BEQ ch1_cont	DEB _ch1_wait_counter	RTS		;are we still waiting
	ch1_cont:	LDR _ch1_pointer		JAS wrSN76489   	INW _ch1_pointer	; tone byte 1
				LDR _ch1_pointer		JAS wrSN76489   	INW _ch1_pointer	; tone byte 2 
				LDR _ch1_pointer		JAS wrSN76489   	INW _ch1_pointer	; volume byte
				LDR _ch1_pointer		CPI 0xff 			BEQ ch1_end			; end of tune
				LDR _ch1_pointer		STB _ch1_wait_counter	INW _ch1_pointer 	RTS
	ch1_end:	CLB _ch1_playing
	ch1_exit:	RTS

_ch1_pointer:		0x0000		; pointer for data read for channel 1
_ch1_playing: 		0			; is channel 1 playing? 1 - yes
_ch1_wait_counter:	0			; decreasing wait counter

					; ******************
audio_channel2:		; * plays queue for channel 2, to be called every frame
					; * data taken from audio_jump table
					; ******************
				LDB	_ch2_playing 		CPI 0 BEQ ch2_exit		; if its not playing then exit
				LDB _ch2_wait_counter	CPI 0 BEQ ch2_cont	DEB _ch2_wait_counter	RTS		;are we still waiting
	ch2_cont:	LDR _ch2_pointer		JAS wrSN76489   	INW _ch2_pointer	; tone byte 1
				LDR _ch2_pointer		JAS wrSN76489   	INW _ch2_pointer	; tone byte 2 
				LDR _ch2_pointer		JAS wrSN76489   	INW _ch2_pointer	; volume byte
				LDR _ch2_pointer		CPI 0xff 			BEQ ch2_end			; end of tune
				LDR _ch2_pointer		STB _ch2_wait_counter	INW _ch2_pointer 	RTS
	ch2_end:	CLB _ch2_playing
	ch2_exit:	RTS

_ch2_pointer:		0x0000		; pointer for data read for channel 2
_ch2_playing: 		0			; is channel 2 playing? 1 - yes
_ch2_wait_counter:	0			; decreasing wait counter

					; ******************
audio_channel3:		; * plays queue for channel 3, to be called every frame
					; * data taken from audio_jump table
					; ******************
				LDB	_ch3_playing 		CPI 0 BEQ ch3_exit		; if its not playing then exit
				LDB _ch3_wait_counter	CPI 0 BEQ ch3_cont	DEB _ch3_wait_counter	RTS		;are we still waiting
	ch3_cont:	LDR _ch3_pointer		JAS wrSN76489   	INW _ch3_pointer	; tone byte 1
				LDR _ch3_pointer		JAS wrSN76489   	INW _ch3_pointer	; tone byte 2 
				LDR _ch3_pointer		JAS wrSN76489   	INW _ch3_pointer	; volume byte
				LDR _ch3_pointer		CPI 0xff 			BEQ ch3_end			; end of tune
				LDR _ch3_pointer		STB _ch3_wait_counter	INW _ch3_pointer 	RTS
	ch3_end:	CLB _ch3_playing
	ch3_exit:	RTS

_ch3_pointer:		0x0000		; pointer for data read for channel 3
_ch3_playing: 		0			; is channel 3 playing? 1 - yes
_ch3_wait_counter:	0			; decreasing wait counter	
