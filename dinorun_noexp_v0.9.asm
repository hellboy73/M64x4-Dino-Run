...
;   *** Change log:                                                                          ***
;   ***  v0.9 13.12.2025 initial working version (no sound, no expansion) emulator friendly  ***
;   ********************************************************************************************
...
clear_buffer:   	; *** clears buffer area 
  ...
  cbuf_loopx:   CLL 0x0000							; clear long word
				...
				DEZ Z1 FGT cbuf_loopy           	; chewck if last line cleared
				RTS			
...
action_loop:
		...
		JPS SpeedCopy							; fast Buffer copy to Playfield
			JPS readJoystickAndKeys
		LDB _dino_state	CPI 5 BEQ game_over		; real game over jump  	_dino_state: 1-still 2-runs 3-jumps 4-ducks 5-dead
...
Wait_fire:			; *** wait fire press and do the blinking if dino is still
	...
	Wf_key_read:
				JPS readJoystickAndKeys
				LDB _up CPI 1 BNE Wait_fire				; PS/2 Input and Clear
		RTS
...
_blink_counter2:	10
					...
					; ******************
Wait_a_sec:			; *** delaying procedure roughly 1 second
					; ******************
	MIB 0xff wait_counter1
	wait_loop1:		LDI 0xff STB wait_counter2
	wait_loop2:		NOP NOP NOP NOP NOP NOP NOP
					DEB wait_counter2
					BNE wait_loop2
					DEB wait_counter1	
					BNE wait_loop1
...
wait_counter2: 0x00
					...
					; ******************
init_pattern:     	; *** fills screen with pattern for better troubleshooting 	
					; *** will be disabled on final release
					; ******************
	MIZ 0 _YPos MIZ 0 _XPos JPS _Print "12345678901234567890123456789012345678901234567890" 0
	MIZ 1 _YPos MIZ 0 _XPos 
	ip_line_loop:
	LDB ip_line_no JAS _PrintChar
		ip_loop:
			LDI 0x97 JAS _PrintChar
			INB ip_counter
			LDB ip_counter CPI 49 BNE ip_loop
		MIB	0 ip_counter
		DEB ip_counter2
		LDB ip_counter2 CPI 0 BEQ ip_exit
		INB ip_line_no 
		LDB ip_line_no CPI ":" BNE ip_line_loop
		MIB "0" ip_line_no
		JPA ip_line_loop
	ip_exit:
		JPS _Print "1234567890123456789012345678901234567890123456789" 0
		RTS
ip_counter:  00           
ip_counter2: 28
ip_line_no: "2"
...
_enemy_x_right:	0		; variable for calculations
						...
						;******************
Control_section:		;* set dino state based on keyboard 
	...
	cs_jump:	LDB	_dino_state	CPI 3	BEQ cs_exit				; is dino during jump? if yes, skip further controls (continue jump)
				LDB _pressed	CPI 0	BEQ cs_other			; is any key pressed? if no set him running 	
...
Enemy1:			;* Enemy One handling procedure (there are two enemies potentially on the screen)
		...
		CIB 20 _enemy2_x_pos BGT	enemy1_exit						; enemy1_x_pos bigger than 20 then exit (enemies too close eachother)
...
Enemy2:			;* Enemy Two handling procedure (there are two enemies potentially on the screen)
	...
	e2_active:
		LDB _enemy2_countdown	CPI 0 	BEQ enemy2_run				; first check if this enemy is on the run (countdown = 0) 
																	; now check if its safe to initiate enemy1 (based on enemy2 position) 
		CIB 20 _enemy1_x_pos BGT	enemy2_exit						; enemy1_x_pos bigger than 20 then exit (enemies too close eachother)
																	; check if a game progressed by one whole byte to dec the countdown
...
_game_level:		0			; game level, starts with 0 level ups after each 100 pts.
						...
						;******************
readJoystickAndKeys:	;* Goofy PS2 handlig routine, to be called more than once in every frame:
						;* check PS2 for any key code, if presed set key variable and pressed variable 
						;* on 0xff return 
						;* on 0xf0 clear pressed and remember to ignore next code from PS2 after one frame
						; ******************
KeyHandler: INK								; PS/2 Input and Clear
			CPI 0xff 	BEQ key_rts
            CPI 0xf0 	BEQ release
            CPI 0x75	BEQ is_up			; cursor up
            CPI 0x72	BEQ is_down			; cursor down
			CPI 0x29	BEQ is_space		; space key
			CPI 0x15	BEQ is_q
key_rts:	RTS
is_up:      LDB _released CPI 1 BEQ release2 LDI 1 STB _up 		LDI 1 STB _pressed CLB _down 	RTS
is_down:    LDB _released CPI 1 BEQ release2 LDI 1 STB _down 	LDI 1 STB _pressed CLB _up 		RTS
is_space: 	LDB _released CPI 1 BEQ release2 LDI 1 STB _up	 	LDI 1 STB _pressed CLB _down	RTS
is_q:		MIB 1 _qpressed	RTS
release:    CLB _pressed 
			LDI 1 STB _released 		RTS	; key release handling
release2: 	CLB _pressed CLB _released 	RTS	; second run of key release handling to clear released key code sent by PS2
